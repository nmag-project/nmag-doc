...
for shape_elem in shape:
    ...
    for N in iteration:
        ...
        pylab.title\
                ("Quality of mesh after %d iterations \n(Time \
        taken to mesh object = %d seconds)" \
                    % (N, timing.seconds()))
        pylab.xlabel("2*inradius/circumradius")
        pylab.ylabel("Number of simplices")
        pylab.hold()
        pylab.savefig("fig_hist_shape_%d_iter%06d.eps"\
                      % (shape_elem,N))
        pylab.ioff
        pylab.hold(False)
...
(Time taken to mesh object = %d seconds)" % (N, timing.seconds()))
            ...
            pylab.hold()
            pylab.savefig\
("fig_hist_shape_%d_badsimplices_iter%06d.eps" % (shape_elem, N)) 
            pylab.ioff
            pylab.hold(False)
...
("Number of high quality simplices after %d iterations\n\
 ...
 % (N, timing.seconds()))
        ...
        pylab.ylabel("Number of simplices")
        pylab.hold()
        pylab.savefig("fig_hist_shape_%d_goodsimplices_iter%06d.eps"\
                      % (shape_elem, N)) #save histogram
        pylab.ioff
        pylab.hold(False)
 ...
 ##   #plot line graph of iteration vs time
    ...
    pylab.savefig("graph_time_shape_%d.eps" % shape_elem)
    ...
    pylab.xlabel("Number of iterations")
    pylab.savefig("graph_node_shape_%d.eps" % shape_elem)
    ...
    pylab.savefig("graph_averagequality_shape_%d.eps" % shape_elem)
    ...
    pylab.ylabel\
        ("Percentage of simplices with ratio within each interval")
    pylab.xlabel("iterations")
    pylab.savefig("graph_quality_shape_%d.eps" % shape_elem)
    pylab.hold()
    ...
    pylab.ylabel\
        ("Percentage of simplices with ratio within each interval")
    pylab.xlabel("iterations")
    pylab.savefig("graph_low_quality_shape_%d.eps" % shape_elem)
    pylab.hold()
