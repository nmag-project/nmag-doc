...
iteration = [1,5,10,20,30,40,50,200,500,1000,2000]
...
for N in iteration:
    ...
    nmesh.visual.plot2d_ps(mesh,"fig_mesh_edge_iter%06d.ps" % N) 
    ...
    #pylab.draw()
    pylab.hold()
    pylab.savefig("fig_hist_edge_iter%06d.eps" %  N) #save histogram
    pylab.ioff
    pylab.hold(False)
    ...
    if len(ratiobad) != 0:
        ...
        #pylab.draw()
        pylab.hold()
        pylab.savefig("fig_hist_edge_badsimplices_iter%06d.eps" %  N) 
        pylab.ioff
        pylab.hold(False)
   ...
   #similary get the good simplices 
    ...
    pylab.title("Number of high quality simplices after %d iterations\n\
    (Time taken to mesh object = %d seconds)" % (N, timing.seconds()))
    pylab.xlabel("2*inradius/circumradius")
    pylab.ylabel("Number of simplices")
    pylab.hold()
    pylab.savefig("fig_hist_edge_goodsimplices_iter%06d.eps" %  N) 
    pylab.ioff
    pylab.hold(False)
...
pylab.savefig("graph_time_edge.eps")
...
pylab.xlabel("Number of iterations")
pylab.savefig("graph_node_edge.eps")
...
pylab.savefig("graph_averagequality_edge.eps")
...
pylab.xlabel("iterations")
pylab.savefig("graph_quality_edge.eps")
pylab.hold()
...
pylab.plot(iteration, stack3, label='25-50%')
pylab.legend()
pylab.title("Low quality Stacks")
pylab.ylabel("Percentage of simplices with ratio within each interval")
pylab.xlabel("iterations")
pylab.savefig("graph_quality_edge_low_quality.eps")
pylab.hold()
