...
for i in range(last_i+1,int(cooling_time/time_step)+last_i+2):
    ...
    T0 = ocaml.probe_field(sundialsbuffer_final,"T",[0.0, 0.0, 0.0])[0][1]
    print "i: %3d T: %s (su), T: %s " % (i,repr(T0),su.conversion_factor_of(SI(1,"K")))
    f.write("%g\t%g\n" % ((heating_time+i*time_step)*su.conversion_factor_of(SI(1,'s')).value,su.conversion_factor_of(SI(1,"K")).value*T0))
